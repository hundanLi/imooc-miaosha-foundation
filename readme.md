...
![下单流程](docs/促销下单流程.png)

①用户下单请求到达后，首先验证用户和秒杀活动等是否有效，如果是则使用促销价格生成订单和扣减库存操作

②如果在生成订单过程中库存不足，则事务回滚，返回下单失败，否则下单成功。

### 2. 关键技术点

| 视频教程                                                     | 自定义                                                       |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| Spring Boot，MyBatis，MyBatis-Generator，MySQL，Hibernate-Validator，html，jQuery，bootstrap，ajax | Spring Boot，MyBatis Plus，MyBatis Plus Generator，MySQL，Hibernate-Validator，Lombok，html，jQuery，bootstrap，ajax |

### 3. 应用分层设计

建议首先根据产品需求文档设计出业务对象模型（Business Object Model，Service层），然后再设计出数据对象模型（Data Object Model，DAO层）。各层的对象应该严格区分开来。但是有时候会出现冗余。

### 4. 数据库设计

数据表设计时要考虑到业务层的需求和数据库的扩展性进行分表。比如用户信息表和密码表应该分开，商品信息是读多写少而商品库存读多写多，因此可以将库存表剥离开，在秒杀过程中更新库存的操作仅仅需要操作库存表，不会影响商品信息表的操作。此外仓库还可能有多个，这样设计起来更加清晰。

数据库主键生成：订单表的主键采用的生成规则是“时间+自增序列+分库分表标记”。这样设计的好处是:

- 包含时间信息：方便订单归档
- 分库分表标记：方便后续的系统扩展。当数据增大以后，可以设计通过路由规则将订单数据保存到不同的库和表中。

### 5. 事务处理

在下单过程中，会涉及新增订单，减库存，增销量等操作，可以使用Spring的声明式事务来管理。

### 6. 遇到的问题

1. 跨域Session

   问题：通过打开本地html文件的方式，chrome浏览器不能跨域使用session，因为默认开启了SameSite=Lax的cookie；但是edge浏览器支持。

   解决方法：

   - 通过不同端口启动应用互相调用来模拟跨域，这样两个浏览器都可以实现跨域使用session。
   - 修改chrome浏览器设置，将SameSite相关的设置disable。
...
**尽量将请求拦截在系统上游**。因为秒杀系统宕机的原因都是数据库的并发能力弱，导致请求响应超时。而且即使流量再大，真正秒杀下单成功的也非常少，因此应该尽量避免无效的数据库操作。
...
1. 静态资源缓存
   ...
   - 页面静态化
     ...
     前后端分离，对商品详情和订单详情页面静态化，动态数据通过接口从服务端加载。

   - 页面缓存

     利用CDN缓存静态资源，或者将渲染的页面缓存到Redis。
...
1. 控制层
   ...
   - 缓存：设置缓存有效时间，在缓存中计数，如果在缓存的有效时间内请求的次数超了的话，就直接返回，拒绝服务。
   - RateLimiter：RateLimiter是guava提供的基于令牌桶算法的限流实现类，通过调整生成token的速率来限制用户频繁访问秒杀页面，从而达到防止过多流量进入服务层。令牌桶算法的原理是系统会以一个恒定的速度往桶里放入令牌，如果请求需要被处理，则需要先从桶里获取一个令牌，当桶里没有令牌可取时，则拒绝服务。
...
2. 服务层
   ...
   **方案**：**本地标记** + **redis预处理** + **MQ异步下单** + **客户端轮询**

   描述：通过三级缓冲保护，1.本地标记， 2.redis预处理， 3.MQ异步下单，最后才会访问数据库，这样做是为了最大力度减少对数据库的访问。
   ...
   3. 为了保护系统不受高流量的冲击而导致系统崩溃的问题，使用MQ异步队列处理下单，作为缓冲保护，实际上做了一个窗口模型，窗口模型会实时的刷新用户秒杀的状态。
...
2. 流程解析
   ...
   ①下单请求到达服务器后，首先检查HashMap存储的本地标记，如果标记无库存，直接返回。否则访问redis预减库存进行锁定，然后将进入支付倒计时。

   ②用户支付成功后，将该请求放进MQ队列中，后台进程轮询队列，将订单入库，扣减数据库的库存等，成功返回。如果用户未及时支付，则关闭订单和释放redis锁定的库存。
   
3. 更详细的流程见：[这里](https://www.cnblogs.com/xiangkejin/p/9351501.html)
